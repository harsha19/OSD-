...
ssize_t ticket_read(struct file *filp, char __user *buf, size_t count, 
	    loff_t *f_pos)
...
ssize_t ticket_write(struct file *filp, const char __user *buf, size_t count, 
	     loff_t *f_pos)
{
  return -EINVAL;
}

loff_t ticket_llseek(struct file *filp, loff_t off, int whence)
{
  return -EINVAL;
}
 ...
 */
static int ticket_construct_device(struct ticket_dev *dev, int minor, 
			struct class *class)
{
  int err = 0;
...
static void ticket_destroy_device(struct ticket_dev *dev, int minor,
		      struct class *class)
{
  BUG_ON(dev == NULL || class == NULL);
...
/* ================================================================ */
static void ticket_cleanup_module(int devices_to_destroy)
{
  int i;
...
static int __init ticket_init_module(void)
{
  int err = 0;
  int i = 0;
...
static void __exit ticket_exit_module(void)
{
